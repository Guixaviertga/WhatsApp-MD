...
// Campos que ocupam muito espaço numa tela estreita. Continuam no arquivo
// de log (que é JSON completo) e reaparecem na tela em nível debug.
const VERBOSE = new Set(['num']);

function formatExtras(entry) {
  const parts = [];
  const showVerbose = entry.level <= 20;
  for (const [key, value] of Object.entries(entry)) {
    if (HIDDEN.has(key)) continue;
    if (VERBOSE.has(key) && !showVerbose) continue;
    if (key === 'err') {
...
// Largura pensada para o terminal de um celular: o nível só aparece
// quando não é INFO (a cor já diferencia), e o módulo é curto.
function formatLine(entry) {
  const level = LEVELS[entry.level] || LEVELS[30];
  const time = paint(clock(entry.time), COLORS.dim);
  const mod = paint((entry.module || '-').slice(0, 8).padEnd(8), COLORS.blue);
  const tag = entry.level === 30 ? '' : `${paint(level.label.trim(), level.color)} `;
  return `${time} ${mod} ${tag}${entry.msg || ''}${formatExtras(entry)}\n`;
}
