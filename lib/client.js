...
async function requestPairingCode(sock, sessionId) {
  ...
  if (!number) {
    throw new Error(
      `Nenhum número de pareamento para a sessão "${sessionId}". `
      + `Defina no .env: PAIRING_NUMBERS=${sessionId}:5511999999999 `
      + '(separe várias sessões por vírgula).',
    );
  }
    ...
    onReconnect: () => {
      ...
      setTimeout(() => {
        startSession(sessionId).catch((err) => {
          logger.error({ err, sessionId }, 'falha ao reconectar');
        });
      }, RECONNECT_DELAY_MS);
    },
    onPairingReady: () => {
