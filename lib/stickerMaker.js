 ...
 *
 * estilo/duracaoMax são repassados a convertToWebp (ver lib/ffmpeg.js).
 */
async function createSticker(buffer, {
  packName, packAuthor, estilo, duracaoMax,
} = {}) {
  const startedAt = Date.now();
  try {
    const webpBuffer = await convertToWebp(buffer, { estilo, duracaoMax });
...
/**
 * Reetiqueta uma figurinha que já é webp (ex.: uma figurinha recebida),
 * sem passar pelo ffmpeg — é só reescrever o EXIF com o pack/autor novo.
 * Usado por ".steal"/".rename".
 */
async function retagSticker(webpBuffer, { packName, packAuthor } = {}) {
  const startedAt = Date.now();
  try {
    const sticker = await writeExif(
      webpBuffer,
    ...
    logger.info({ ms: Date.now() - startedAt }, 'figurinha reetiquetada');
    return sticker;
  } catch (err) {
    logger.error({ err, ms: Date.now() - startedAt }, 'falha ao reetiquetar figurinha');
    return null;
  }
}

module.exports = { createSticker, retagSticker };
