 ...
 */
function mascaraLavfi(estilo, tamanho) {
...
function buildArgs({
  ...
  const filtro = filtroCor(estilo, tamanho);
  const args = ['-y', '-i', inputPath];

  if (comMascara) args.push('-f', 'lavfi', '-i', mascaraLavfi(estilo, tamanho));

  args.push('-t', String(duracaoMax));

  if (comMascara) {
    const cadeiaVideo = estatica ? filtro : `${filtro},fps=${fps}`;
    // O fps vai explicitamente NOS DOIS ramos. A fonte lavfi da máscara
    // roda a 25fps por padrão, e o alphamerge casa dois streams de taxas
    // diferentes gerando saída na taxa mais alta — o que encheria a
    // figurinha de frames duplicados (25fps em vez dos 5-10 pedidos).
    // Se isso acontece ou não depende da versão do ffmpeg, então em vez de
    // confiar no padrão a gente fixa os dois lados.
    const cadeiaMascara = estatica
      ? 'loop=loop=-1:size=1:start=0'
      : `loop=loop=-1:size=1:start=0,fps=${fps}`;
    args.push(
      '-filter_complex',
      `[0:v]${cadeiaVideo}[cor];[1:v]${cadeiaMascara}[masc];[cor][masc]alphamerge[out]`,
      '-map', '[out]',
    );
  } else {
