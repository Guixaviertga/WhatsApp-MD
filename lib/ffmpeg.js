...
function isImagemEstatica(buffer) {
  ...
  if (isWebp(buffer)) return !isWebpAnimado(buffer);
  return false;
}
...
/**
 * Um webp é animado quando traz o bloco "VP8X" com o bit de animação
 * ligado. Distinguir importa porque uma figurinha comum (webp de 1 frame,
 * o caso de responder ".sticker" a outra figurinha) é imagem estática,
 * enquanto uma figurinha animada tem que seguir pelo caminho de vídeo.
 */
function isWebpAnimado(buffer) {
...
async function convertToWebp(inputBuffer, { estilo = 'crop', duracaoMax = config.stickerMaxDuration } = {}) {
  ...
  const comoEstatica = async () => tentarConversao({
    estilo, estatica: true, duracaoMax, inputPath, ...RUNG_ESTATICA,
  });
  ...
  try {
    ...
    if (estatica) {
      const resultado = await comoEstatica();
    ...
    // Nenhuma tentativa gerou nada: o arquivo provavelmente tem 1 frame só
    // e a assinatura não o revelou como imagem — aí o filtro "fps" descarta
    // esse único frame e a saída sai vazia. Já aconteceu duas vezes, com
    // formatos diferentes (JPEG e depois webp), então em vez de mapear todo
    // formato possível, tenta de novo pelo caminho de imagem.
    logger.warn('nenhuma tentativa de vídeo funcionou — tentando como imagem estática');
    const resultado = await comoEstatica();
    if (resultado.ok) return resultado.buffer;

    throw ultimoErro || resultado.err || new Error('não foi possível gerar a figurinha.');
  } finally {
