 ...
 */
function buildArgs({
  filtro, estatica, duracaoMax, qualidade, fps,
}) {
  return [
    '-y',
    '-i', 'pipe:0',
...
async function convertToWebp(inputBuffer, { estilo = 'crop', duracaoMax = 10 } = {}) {
  ...
  if (estatica) {
    const resultado = await tentarConversao(inputBuffer, {
      filtro, estatica, duracaoMax, ...ESCADA_QUALIDADE[0],
    });
  ...
  const resultados = await Promise.all(
    ESCADA_QUALIDADE.map((rung) => tentarConversao(inputBuffer, { filtro, estatica, duracaoMax, ...rung })),
  );
