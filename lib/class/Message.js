...
class Message extends Base {
  ...
  get contextInfo() {
    const content = this.raw.message || {};
  ...
  /** JIDs mencionados (@alguém) na mensagem. */
  get mentions() {
    return this.contextInfo?.mentionedJid || [];
  }

  /** Quem enviou a mensagem que esta responde, se houver. */
  get quotedSender() {
    const info = this.contextInfo;
    return info?.quotedMessage ? (info.participant || null) : null;
  }

  /** Texto da mensagem respondida, se houver. */
  get quotedText() {
    const quoted = this.contextInfo?.quotedMessage;
    return quoted ? extractText({ message: quoted }) : '';
  }
